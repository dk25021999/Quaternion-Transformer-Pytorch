...
class StackedLSTM(nn.Module):
    def __init__(self, feat_size, hidden_size, use_cuda, n_layers, batch_first=True):
        ...
        self.batch_first = batch_first
        self.layers = nn.ModuleList([LSTM(feat_size, hidden_size, use_cuda) for _ in range(n_layers)])

    def forward(self, x):
        # LSTM takes inputs of shape (seq_len, batch_size, feat_size)
        if self.batch_first:
            x = x.permute(1,0,2)
        for layer in self.layers:
            x = layer(x)
            # Remove extra feature added by LSTM
            x = x[:,:,:-1]
        if self.batch_first:
            x = x.permute(1,0,2)
        return x
